  ...
  return (
    <div className="bg-secondary h-min-screen py-3">
      ...
      <div id="sidebar" className="flex flex-col gap-y-4 px-2 md-px:1 lg:mt-4">
        ...
        {role == 'chargeNurse' && <div className="flex flex-row items-center border-b-2 border-gray">
          <div className="text-lg font-extrabold tracking-wide text-blue-900 lg:text-xl">
            Nurses
          </div>
        </div>}
        {role == 'chargeNurse' && <CustomLink tabs={nurseItems(empId)} />}
        {role == 'doctor' && <div className="flex flex-row items-center border-b-2 border-gray">
          <div className="text-lg font-extrabold tracking-wide text-blue-900 lg:text-xl">
            Doctors
          </div>
        </div>}
        {role == 'doctor' && <CustomLink tabs={doctorItems(empId)} />}
        <div className="flex flex-row items-center border-b-2 border-gray">
          <div className="text-lg font-extrabold tracking-wide text-blue-900 lg:text-xl">
            General
          </div>
        </div>
        <CustomLink tabs={generalItems(empId)} />
      </div>
    </div>
  );
}
