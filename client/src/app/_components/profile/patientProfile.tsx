...
import { getPatient, updatePatient } from "@/app/_lib/api";
...
const PatientProfile = ({patientId, onSubmit=null}) => {
  ...
  useEffect(() => {
    getPatient(user.employeeNumber, patientId).then((result) => {
      console.log(result);
      if (result) {
  ...
  const handleSaveClick = () => {
    ...
    updatePatient(user.employeeNumber, patientId, editedValues).then((data) => {
      console.log(data);
    }).catch((error) => {
      console.log(error);
    });
            ...
            <div className="grid grid-cols-1">
              <div className="text-gray-700 text-sm">
                ...
                <div className="grid grid-cols-2">
                  ...
                  <div className="py-2">{editedValues.nokName}</div>
                </div>
                <div className="grid grid-cols-2">
                  <div className=" py-2 font-semibold">Relations</div>
                  <div className=" py-2">{editedValues.nokRelation}</div>
                </div>
                <div className="grid grid-cols-2">
                  <div className="py-2 font-semibold">Phone Number.</div>
                  <div className="py-2">{editedValues.nokPhoneNum}</div>
                </div>
                <div className="grid grid-cols-2">
                  <div className=" py-2 font-semibold">Address</div>
                  <div className="py-2">{editedValues.nokStreet + ", " + editedValues.nokCity}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-3 shadow-sm rounded-sm">
            ...
            <div className="text-gray-700">
              <div className="grid md:grid-cols-1 text-md">
                <div className="grid grid-cols-2">
                  <div className="px-4 py-2 font-semibold">Insurance Number:</div>
                  <div className="px-4 py-2">{editedValues.insuranceNum}</div>
                </div>
                <div className="grid grid-cols-2">
                ...
                <div className="grid grid-cols-2">
                  ...
                  {isEditing ? (
                    ...
                    <div className="px-4 py-2">{editedValues.dateOfBirth}</div>
                  )}
                </div>
                <div className="grid grid-cols-2">
                ...
                <div className="grid grid-cols-2">
                  ...
                  {isEditing ? (
                    <input
                      type="text"
                      name="phoneNumber"
                      value={editedValues.phoneNum}
                      onChange={handleChange}
                      className="px-4 py-2"
                    />
                  ) : (
                    <div className="px-4 py-2">{editedValues.phoneNum}</div>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1">
                  ...
                  {isEditing ? (
                    ...
                    <div className=" py-2">
                      {editedValues.street},{" "}
                      {editedValues.city}, {editedValues.postalCode}
                    </div>
                  )}
                </div>
              </div>
